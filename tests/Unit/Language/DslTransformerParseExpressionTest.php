...
it('parses simple direct user type references correctly', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
    ...
    // Check viewer relation metadata with wildcard
    ...
    // Check that we have both user types
    $types = [];
    ...
    // Should have 2 types: user and user:*
    expect($types)->toHaveCount(2);

    // Check that at least one has type 'user'
    $hasUserType = false;
...
it('parses simple userset references correctly', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
    ...
    // Check metadata shows no directly related user types for computed usersets
    $metadata = $documentType->getMetadata();
    $relationsMetadata = $metadata->getRelations();
...
it('parses union expressions correctly with metadata', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
...
it('parses intersection expressions correctly with metadata', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
...
it('parses exclusion expressions correctly with metadata', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
...
it('parses nested and grouped expressions correctly', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
...
it('parses tuple-to-userset patterns correctly', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
...
it('handles operator precedence correctly', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
    ...
    // Should parse as: a or (b and (c but not d))
    // Due to precedence: but not > and > or
    $relations = $documentType->getRelations();
    $complexRelation = $relations->get('complex');

    // Top level should be union (or)
    ...
    // Second child should be intersection (and)
    ...
    // First intersection child should be 'b'
    ...
    // Second intersection child should be difference (but not)
...
it('preserves metadata correctly for complex expressions', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
...
it('handles self keyword correctly', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
...
it('handles empty types correctly', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
    ...
    // Metadata might be null for empty types
    // Just check that it exists or not, don't assert either way
    // as the implementation may vary
    $emptyType->getMetadata();
});

it('handles mixed direct and computed usersets in unions correctly', function (): void {
    $dsl = <<<'DSL'
        model
          schema 1.1
    ...
    // Should have user and group types
    $typeNames = [];
