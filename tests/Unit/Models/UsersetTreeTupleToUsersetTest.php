...
use OpenFGA\Models\{Leaf, Node, UsersetTreeTupleToUserset};
...
test('can create with base and subtract nodes', function (): void {
    ...
    $tupleToUserset = new UsersetTreeTupleToUserset($baseNode, $subtractNode);

    expect($tupleToUserset->getBase())->toBe($baseNode)
        ->and($tupleToUserset->getSubtract())->toBe($subtractNode);
});
...
test('json serialize returns correct structure', function (): void {
    ...
    $result = $tupleToUserset->jsonSerialize();
