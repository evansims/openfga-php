...
it('can create Store from array', function () {
    ...
    expect($store)->toBeInstanceOf(StoreInterface::class)
        ...
        ->and($store->createdAt->setTimezone(new \DateTimeZone('UTC'))->format(DATE_ATOM))->toBe($data['created_at'])
        ->and($store->updatedAt->setTimezone(new \DateTimeZone('UTC'))->format(DATE_ATOM))->toBe($data['updated_at'])
        ->and($store->deletedAt)->toBeNull();
});
...
it('can create Store from array with deletedAt', function () {
    ...
    expect($store->deletedAt->setTimezone(new \DateTimeZone('UTC'))->format(DATE_ATOM))->toBe($data['deleted_at']);
});
